...
class MotorWidget extends BaseWidget {
  constructor() {
    super(...arguments);
    __publicField(this, "_svg", null);
    __publicField(this, "_body", null);
    __publicField(this, "_indicator", null);
    __publicField(this, "_rotateAnim", null);
  }
  render() {
    this.innerHTML = "";
    ...
    this._labelElement = this.createLabelElement(this._widget?.properties.label ?? "MOTOR", this.getLabelSide());
    this.appendChild(this._labelElement);
    this.updateVisuals();
  }
  updateVisuals() {
    if (!this._body || !this._widget) return;
    this.stopBlink();
    this.stopPulse();
