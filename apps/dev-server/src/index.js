...
app.post("/api/tags/values", (req, res) => {
  ...
  const result = tagIds.map((id) => MOCK_TAGS.find((t) => t.tagId === id)).filter((tag) => tag !== void 0).map((tag) => ({ tagId: tag.tagId, value: computeValue(tag, elapsed), timestamp: Date.now() }));
  res.json(result);
});
...
wss.on("connection", (ws) => {
  clients.add(ws);
  subscriptions.set(ws, /* @__PURE__ */ new Set());
...
const tagIntervals = /* @__PURE__ */ new Map();
