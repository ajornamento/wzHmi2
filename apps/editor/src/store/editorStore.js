...
function pushHistory(state, newSchema) {
...
const useEditorStore = create((set, get) => ({
  ...
  addWidget: (type, customProperties, customGeometry) => set((s) => {
    ...
    const snapTo10 = (v) => Math.round(v / 10) * 10;
  ...
  removeWidget: (id) => set((s) => {
    const newWidgets = s.schema.widgets.filter((w) => w.id !== id).map((w) => {
      ...
      return {
        ...
        properties: {
          ...w.properties,
          ...sc?.widgetId === id ? { startConnection: void 0 } : {},
          ...ec?.widgetId === id ? { endConnection: void 0 } : {}
        }
      };
    });
  ...
  updateWidget: (id, patch) => set((s) => {
    const newWidgets = s.schema.widgets.map(
      (w) => w.id === id ? mergeWidget(w, patch) : w
    );
    ...
    const newWidgets = s.schema.widgets.map((w) => {
      if (w.id === id) {
        if (w.type === "LINE") {
          ...
          const wps2 = (w.properties.waypoints ?? []).map((wp) => ({ x: wp.x + dx, y: wp.y + dy }));
          return {
            ...w,
    ...
    const newWidgets = s.schema.widgets.map((w) => {
      ...
      return {
        ...
        properties: {
          ...
          x2,
          y2,
          ...endpoint === "start" ? { startConnection: connection ?? void 0 } : { endConnection: connection ?? void 0 }
        }
      };
    });
    return pushHistory(s, { ...s.schema, widgets: newWidgets });
  ...
  moveLineWaypoint: (id, index, x, y) => set((s) => {
    const newWidgets = s.schema.widgets.map((w) => {
      ...
      const x1 = Number(w.properties.x1 ?? 0), y1 = Number(w.properties.y1 ?? 0);
      const x2 = Number(w.properties.x2 ?? 0), y2 = Number(w.properties.y2 ?? 0);
      const wps = (w.properties.waypoints ?? []).map(
        (wp, i) => i === index ? { x: Math.round(x), y: Math.round(y) } : wp
      );
      return {
        ...w,
        geometry: { ...w.geometry, ...recalcLineGeometry(x1, y1, x2, y2, wps) },
  ...
  resizeWidget: (id, width, height) => set((s) => {
    const newWidgets = s.schema.widgets.map(
      (w) => w.id === id ? { ...w, geometry: { ...w.geometry, width: Math.max(20, Math.round(width)), height: Math.max(20, Math.round(height)) } } : w
    );
    return { schema: { ...s.schema, widgets: newWidgets } };
  }),
  ...
  bringForward: (id) => set((s) => {
    const w = s.schema.widgets.find((w2) => w2.id === id);
    if (!w) return {};
    const newWidgets = s.schema.widgets.map(
      (ww) => ww.id === id ? { ...ww, geometry: { ...ww.geometry, zIndex: ww.geometry.zIndex + 1 } } : ww
    );
    return pushHistory(s, { ...s.schema, widgets: newWidgets });
  }),
  sendBackward: (id) => set((s) => {
    ...
    if (!w) return {};
    const newWidgets = s.schema.widgets.map(
      (ww) => ww.id === id ? { ...ww, geometry: { ...ww.geometry, zIndex: Math.max(1, ww.geometry.zIndex - 1) } } : ww
    );
    return pushHistory(s, { ...s.schema, widgets: newWidgets });
  }),
